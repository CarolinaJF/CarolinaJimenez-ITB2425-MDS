...
def extraer_incidencias(xml_file, json_file):
    ...
    if not os.path.exists(xml_file):
        print("Error: No se encontró el archivo XML. No hay resultados.")
        return

    # Intentar cargar y parsear el archivo XML
    try:
        tree = ET.parse(xml_file)
        root = tree.getroot()
    except ET.ParseError:
        print("Error: El archivo XML está corrupto o no se pudo parsear.")
        return
    ...
    resultados = {
        ...
        "timestamp": datetime.now().isoformat()  # Añadir el timestamp aquí
    }
